...
export const useVillaStore = create<VillaState>((set, get) => ({
  ...
  updateVilla: async (id, updates) => {
    ...
    try {
      ...
      const { error, data } = await supabase.from("villas").update(normalizedUpdates).eq("id", id).select().single()
...
function normalizeVillaData(data: any): any {
  ...
  // Number fields
  if (data.price_per_night !== undefined) normalized.price_per_night = Number.parseInt(data.price_per_night, 10)
  if (data.cleaning_fee !== undefined) normalized.cleaning_fee = Number.parseInt(data.cleaning_fee, 10)
  if (data.service_fee !== undefined) normalized.service_fee = Number.parseInt(data.service_fee, 10)
  if (data.capacity !== undefined) normalized.capacity = Number.parseInt(data.capacity, 10)
  if (data.bedrooms !== undefined) normalized.bedrooms = Number.parseInt(data.bedrooms, 10)
  if (data.bathrooms !== undefined) normalized.bathrooms = Number.parseInt(data.bathrooms, 10)
  if (data.minimum_stay !== undefined) normalized.minimum_stay = Number.parseInt(data.minimum_stay, 10)
  if (data.rating !== undefined) normalized.rating = Number.parseFloat(data.rating)
  if (data.review_count !== undefined) normalized.review_count = Number.parseInt(data.review_count, 10)
  ...
  if (data.coordinates !== undefined) {
    if (data.coordinates && typeof data.coordinates === "object") {
      normalized.coordinates = {
        lat: Number.parseFloat(data.coordinates.lat) || 0,
        lng: Number.parseFloat(data.coordinates.lng) || 0,
      }
    } else {
