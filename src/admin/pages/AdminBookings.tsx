...
const BookingDetailModal = ({
  ...
  const { cancelBookingInSupabase, updateBookingStatus } = useBookingStore()
            ...
            <div className="space-y-6">
              ...
              <div className="admin-card p-4">
                ...
                <div className="space-y-3">
                  ...
                  <button onClick={handleContactGuest} className="w-full admin-btn admin-btn-outline mt-2">
                    <Mail size={16} />
                    Contact Guest
                  </button>
                </div>
...
const ManualBookingModal = ({
  ...
  const handleCreate = () => {
    toast({
      ...
      description: "Manual booking creation backend API needed.",
      variant: "destructive",
    })
    onClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg">
        ...
        <p className="mb-4 text-gray-600">
          This feature requires a backend endpoint to create bookings without payment.
        </p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => setManualBookingOpen(true)} className="admin-btn admin-btn-primary">
            <Plus size={18} />
            Manual Booking
          </button>
        </div>
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              activeTab === tab.key ? "bg-[#778873] text-white" : "bg-[#F1F3E0] text-[#2d3a29] hover:bg-[#D2DCB6]"
            }`}
          >
            {tab.label}
          </button>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-4 border-t border-[#d4dbc8]">
            <p className="text-sm text-[#6b7c67]">
              Showing {(currentPage - 1) * perPage + 1} to {Math.min(currentPage * perPage, filteredBookings.length)} of{" "}
      ...
      />
      <ManualBookingModal isOpen={manualBookingOpen} onClose={() => setManualBookingOpen(false)} />
    </div>
  )
}
