  ...
  const handleSave = async (data: Partial<Villa>) => {
    ...
    try {
      ...
      const saveData = {
        ...
        coordinates: data.coordinates || { lat: -8.5069, lng: 115.2625 },
      }

      if (selectedVilla) {
        console.log("[v0] Updating villa:", selectedVilla.id)
        console.log("[v0] Save data:", saveData)

        const result = await updateVilla(selectedVilla.id, saveData)
        ...
        if (result.success) {
          ...
          console.error("[v0] Update failed with error:", result.error)
          toast({
            title: "Update failed",
            description: result.error || "Failed to update villa",
            variant: "destructive",
          })
        }
      } else {
        console.log("[v0] Creating new villa:", saveData)
