...
const amenityOptions = [
    'WiFi', 'Air Conditioning', 'Pool', 'Kitchen', 'Parking', 'TV',
    'Washing Machine', 'Garden', 'BBQ', 'Safe', 'Gym', 'Spa',
    'Ocean View', 'Mountain View', 'Balcony', 'Fireplace',
];
        ...
        <div className="admin-card overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="relative h-48 group">
                ...
                <div className="absolute top-3 right-3">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${villa.is_available
            ...
            <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                    ...
                    <button
                        ...
                        className={`relative w-12 h-6 rounded-full transition-colors ${villa.is_available ? 'bg-[#A1BC98]' : 'bg-gray-300'
                            }`}
                    >
                        <div
                            className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${villa.is_available ? 'translate-x-7' : 'translate-x-1'
                                }`}
                        />
                    </button>
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-[#d4dbc8]">
                    <button
                        onClick={onEdit}
                        className="flex-1 admin-btn admin-btn-outline py-2 text-sm"
                    >
                        <Edit2 size={14} />
                        Edit
                    </button>
                ...
                <div className="flex border-b border-[#d4dbc8] overflow-x-auto">
                    {tabs.map((tab, index) => (
                        <button
                            ...
                            className={`px-6 py-3 text-sm font-medium whitespace-nowrap transition-colors ${activeTab === index
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {amenityOptions.map((amenity) => (
                                    <label
                                        key={amenity}
                                        className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-colors ${formData.amenities.includes(amenity)
